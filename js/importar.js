...
function start() {
	...
	if (sum > jason.allq.questao.length) {
 	...
 	document.open();
 		...
 		document.write(`<div id="main1"><label>Questão ${l}</label><br><input type="text" name="qs${l}" id="qs${l}" value="${eval(jason.allq.questao[i].title)}" class="gr"><br>`)
		...
		var ilink = ""
 		...
 		for (j = 0; j < q2.value; j++) {
 	 	...
 	 	document.write(`<input type="text" name="qs${l}${k}" id="qs${l}${k}" value="${eval(gambi)}" class="gr"><br>`)
 	 	}
 	 	for (j = 0; j < q2.value; j++) {
 	 	var k = String.fromCharCode(97 + j).toUpperCase();
 	 	...
 	 	document.write('</div>')
 	 	worka = eval(`"#correta${l}${jason.allq.questao[i].gabarito}"`) 	 	
 	 	console.log(worka)
 	 	comandos.push(worka);
	...
	for (var i = 0; i < comandos.length; i++) {
		document.querySelector(comandos[i]).checked = true
	}
